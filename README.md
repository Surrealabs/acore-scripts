...
A complete custom UI suite for AzerothCore WoTLK 3.3.5a servers, built with [mod-ssui (Surreal Sync UI, a fork of Rochet2's AIO)](https://github.com/Rochet2/AIO) and [Eluna (mod-ale)](https://github.com/azerothcore/mod-eluna). See `wiki/ssui.md` for how the fork's client-load caching differs from vanilla AIO.
...
| `SurrealActionBar_AIO.lua` / `SurrealActionBarConfig_AIO.lua` | — / `/actionbarreset` | Single fixed 12-slot action bar (reuses Blizzard's real action buttons, so it works fully in combat). Loadout per class/spec and proc-triggered icon-swap + sparkle glow rules are GM-authored via the SDBEditor "Action Bar Builder" web tool. Players' manual slot edits are saved server-side and take precedence until `/actionbarreset`. Blizzard's default bar/multibars/stance bar are permanently hidden. `SurrealActionBarConfig_AIO.lua` is auto-generated by the web tool's Deploy button — do not edit by hand. |
...
2. Ensure the following modules are installed:
   ...
   - [mod-ssui](https://github.com/Rochet2/AIO) — Server→client Lua push system (Surrealabs' fork of AIO, renamed from mod-aio)
